...
#include "eio.h"
#define EIO_RELEASE "SFSMARTCACHE"
...
int eio_version_query(size_t buf_sz, char *bufp)
{
	if (unlikely(buf_sz == 0) || unlikely(bufp == NULL))
		return -EINVAL;
	snprintf(bufp, buf_sz, "SF-Smartcache Version: %s %s (checksum disabled)",
...
#define PROC_STR                "sfsmartcache"
#define PROC_VER_STR            "sfsmartcache/version"
...
#define PROC_SYS_DIR_NAME               "sfsmartcache"
#define PROC_SYS_CACHE_NAME             "sfsmartcache-dev"
